    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                local tmp = self.highlightedTile
                ...
                :finish(function()
                    local pscore = self.score
                    self:calculateMatches()
                    if pscore == self.score then
                        gSounds['error']:play()
                        tempX = tmp.gridX
                        tempY = tmp.gridY
    
                        tmp.gridX = newTile.gridX
                        tmp.gridY = newTile.gridY

                        newTile.gridX = tempX
                        newTile.gridY = tempY
    
                        self.board.tiles[tmp.gridY][tmp.gridX] =
                        tmp
    
                        self.board.tiles[newTile.gridY][newTile.gridX] = newTile

                        Timer.tween(0.2, {
                            [tmp] = {x = newTile.x, y = newTile.y},
                            [newTile] = {x = tmp.x, y = tmp.y}
                        })
                    end
                    if not self.board:possibleMatch() then
                        gSounds['reset']:play()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for j, tile in pairs(match) do
                self.score = self.score + 50 + (tile.variety-1)*10
                self.timer = self.timer + 1
            end
        end
...
function PlayState:render()
   ...
   if self.test then
    ...
    if self.rectHighlighted then
        ...
        if self.highlightedTile ~= nil and self.highlightedTile.shiny then
